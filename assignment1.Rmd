...
con = RPostgres::dbConnect(
  RPostgres::Postgres(),
  dbname = "rollsrobin_databases",
  host = 'ddcmstud.ugent.be',
  port = 8088,
  user = "sql_exerciser",
...
```{r, include=F}
library(tidyverse)
kable = function(data, scroll_box=F) {
  response = knitr::kable(data, booktabs = TRUE, digits = 2)
  if(scroll_box){
    response=response %>% 
...
(2) If two different names in `branch.name` are considered two different branches, then the answer is 'yes, multiple locations can be assigned to one branch'. There is no constraint on the `branch.name` column, so two rows might exist with identical `branch.name`.

Every enterprise -or row- in the `branch` table has a location given by `branch.postalcode` and `branch.municipality`. If there are two rows with the same name and different locations, then we have a situation where multiple locations are assigned to the same branch.
...
Next, we know from the relational schema that locations are stored for the branches as well as for the employee working for those branches. If every location must be linked to a branch, than branch employees can only live in the location of a branch, which would be quite stupid.
...
If this assumption is true, than in the result of the query we will see identical numberplates linked to multiple distinct carmodels, which shouldn't be possible. I looked around and qquickly found proof for this hypthesis:
...
disadvantage: the `employeenumber` in the `person`-table must become optional, as basic csutomers will not have an employeenumber. In that sense, it is crucial that an employee can only be made with an employeenumber, which is a check that has to be done by the form which creates new employees.
 
**Answer**: I would advise against the integration of the `employee` and `person` tables
...
```{sql first exercise,code=readLines("01504786_Eli_Lauwers_1.sql"),connection=con,output.var="df.1"}
...
```{sql second exercise,code=readLines("01504786_Eli_Lauwers_2.sql"),connection=con,output.var="df.2"}
...
```{sql third exercise,code=readLines("01504786_Eli_Lauwers_3.sql"),connection=con,output.var="df.3"}
...
```{sql fourth exercise,code=readLines("01504786_Eli_Lauwers_4.sql"),connection=con,output.var="df.4"}
...
```{sql fifth exercise,code=readLines("01504786_Eli_Lauwers_5.sql"),connection=con,output.var="df.5"}
